...
with open("transcription_test.pkl", "rb") as f:
...
def mean_accuracy(y_true, y_pred):
    return 1 - np.mean(np.abs(np.array(y_true) - np.array(y_pred)))

def concordance_correlation_coefficient(y_true, y_pred):
    mean_true = np.mean(y_true)
    mean_pred = np.mean(y_pred)
    cov = np.mean((np.array(y_true) - mean_true) * (np.array(y_pred) - mean_pred))
...
def plot_comparison(maccs, cccs):
    labels = ["Zero-shot", "One-shot"]
    x = range(len(labels))
    width = 0.35

    fig, ax = plt.subplots(figsize=(8, 5))
    ax.bar([i - width/2 for i in x], maccs, width, label="mAcc", color="dodgerblue")
    ax.bar([i + width/2 for i in x], cccs, width, label="CCC", color="seagreen")

    ax.set_ylabel("Score")
    ax.set_title("Zero-shot vs One-shot performance (5 runs averaged)")
    ax.set_xticks(x)
    ...
    for i in x:
        ax.text(i - width/2, maccs[i] + 0.02, f"{maccs[i]:.3f}", ha="center")
        ax.text(i + width/2, cccs[i] + 0.02, f"{cccs[i]:.3f}", ha="center")

    plt.tight_layout()
    plt.savefig("qwen25_comparison.png")
    plt.show()

print("Running zero-shot on test...")
preds_zero = run_experiment(zero_shot_prompt, transcripts_test, runs=5)
save_json(preds_zero, "qwen25_zero_test.json")

print("Running one-shot on test...")
preds_one = run_experiment(one_shot_prompt, transcripts_test, runs=4)
save_json(preds_one, "qwen25_one_test.json")

macc_zero, ccc_zero = evaluate(preds_zero, true_scores_test)
macc_one, ccc_one = evaluate(preds_one, true_scores_test)

print(f"\nZero-shot mAcc: {macc_zero:.4f}, CCC: {ccc_zero:.4f}")
print(f"One-shot  mAcc: {macc_one:.4f}, CCC: {ccc_one:.4f}")

with open("qwen25_zero_test.json", "r") as f:
    raw_zero = json.load(f)

with open("qwen25_one_test.json", "r") as f:
    raw_one = json.load(f)

def average_predictions(runs_dict):
    averaged_scores = {}
    for clip_id, runs in runs_dict.items():
        processed_runs = []
        for r in runs:
            if isinstance(r, str):
                try:
        ...
        for trait in traits:
            vals = [
                safe_float(r.get(trait))
                for r in processed_runs
                if isinstance(r, dict) and trait in r and safe_float(r.get(trait)) is not None
            ]
            if vals:
                avg[trait] = float(np.mean(vals))
        averaged_scores[clip_id] = avg
...
preds_zero = average_predictions(raw_zero)
preds_one = average_predictions(raw_one)

macc_zero, ccc_zero = evaluate(preds_zero, true_scores_test)
macc_one, ccc_one = evaluate(preds_one, true_scores_test)

print(f"Zero-shot mAcc: {macc_zero:.4f}, CCC: {ccc_zero:.4f}")
print(f"One-shot  mAcc: {macc_one:.4f}, CCC: {ccc_one:.4f}")
